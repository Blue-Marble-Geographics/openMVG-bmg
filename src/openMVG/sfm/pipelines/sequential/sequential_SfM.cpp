  ...
  std::vector<uint32_t> & vec_possible_indexes)
{
  ...
  // Collect tracksIds (sorted vector; landmarks is a Hash_Map so sort explicitly).
  std::vector<uint32_t> reconstructed_trackId;
  ...
  vec_putative.reserve(set_remaining_view_id_.size());
#ifdef OPENMVG_USE_OPENMP
  #pragma omp parallel
#endif
...
bool SequentialSfMReconstructionEngine::Resection(const uint32_t viewIndex)
{
  ...
  // A2. intersects the track list with the reconstructed (sorted vector;
  // landmarks is a Hash_Map so sort explicitly).
  std::vector<uint32_t> reconstructed_trackId;
  reconstructed_trackId.reserve(sfm_data_.GetLandmarks().size());
  std::transform(sfm_data_.GetLandmarks().cbegin(), sfm_data_.GetLandmarks().cend(),
    std::back_inserter(reconstructed_trackId),
  ...
  // Get the ids of the already reconstructed tracks (sorted vector).
  std::vector<uint32_t> set_trackIdForResection;
  set_trackIdForResection.reserve(std::min(set_tracksIds.size(), reconstructed_trackId.size()));
  std::set_intersection(set_tracksIds.cbegin(), set_tracksIds.cend(),
    reconstructed_trackId.cbegin(), reconstructed_trackId.cend(),
    std::back_inserter(set_trackIdForResection));
  ...
  std::vector<uint32_t>::const_iterator iterTrackId = set_trackIdForResection.begin();
